...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{wflow_start}\NormalTok{(}\StringTok{"PROJECT"}\NormalTok{)}
\KeywordTok{wflow_build}\NormalTok{()}
\KeywordTok{wflow_publish}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"analysis/*.Rmd"}\NormalTok{), }\StringTok{"Publish the initial files for PROJECT"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\hypertarget{connecting-to-gitlab}{%
\section{Connecting to GitLab}\label{connecting-to-gitlab}}
...
\item
  Make sure you are in PROJECT directory and paste into \texttt{Terminal}
\end{itemize}

\hypertarget{creating-a-new-file}{%
\section{Creating a New File}\label{creating-a-new-file}}
